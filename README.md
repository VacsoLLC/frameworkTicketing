...
## Deploy to shared vacso dev (AWS)

Alternative to the docker-compose install above: deploy onto the shared
AWS infrastructure in `/workspaces/shared` (shared ECS cluster, shared
ALB, shared MariaDB RDS, `dev.vacso.com` Route53 zone). This mirrors the
pattern used by `/workspaces/casenet`.

Target: `https://ticketing.dev.vacso.com`

### Prerequisites

- AWS CLI configured for the vacso dev account
- `terraform` >= 1.5, `docker`, `yarn`, `jq`, `mysql` client
- Shared infrastructure already deployed (`cd /workspaces/shared && terraform apply`)

### 1. Apply consumer terraform
...
This provisions: ECR repo, ACM cert (us-east-1, DNS-validated), S3
bucket + CloudFront distribution, Route53 A/AAAA records, Secrets
Manager entry (`SECRET` + `ADMIN_PASSWORD` auto-generated, email/SAML
left as empty stubs), ECS task definition + service, and an ALB
listener rule for `ticketing.dev.vacso.com`.

### 2. Bootstrap the database (one-time)

The shared RDS instance exists, but the `core` and `ticketing`
databases need to be created once. Open the SSH tunnel via the shared
repo and run the bootstrap SQL:

```bash
# Terminal 1 — open the tunnel from the shared repo
cd /workspaces/shared
eval "$(terraform output -raw db_tunnel_command)"

# Terminal 2 — create the databases
DB_PASS=$(aws secretsmanager get-secret-value \
  --secret-id shared-dev/db-credentials \
  --query SecretString --output text | jq -r '.password')
mysql -h 127.0.0.1 -u admin -p"$DB_PASS" < scripts/bootstrap-db.sql
```

The framework backend auto-creates tables inside these databases on
first startup.
...
- Backend: builds `docker/Dockerfile.ecs`, pushes to ECR, forces an ECS
  redeploy, waits for the service to stabilize.
- Frontend: runs `yarn build` in `frontend/`, syncs `dist/` to S3 with
  appropriate cache headers, invalidates CloudFront.

### 4. Verify

```bash
curl https://ticketing.dev.vacso.com/api/core/healthcheck/status
# → "API is healthy"
```
...
Then log in at `https://ticketing.dev.vacso.com` as `admin`.

### What's configured (and what isn't)

- TLS: handled by CloudFront (ACM cert in us-east-1). The shared ALB
  stays HTTP-only; CloudFront→ALB is HTTP.
- Email (Microsoft/Google), SAML SSO, user sync: left disabled with
  empty env stubs. To enable, update the `*-app-secrets` entry in
  Secrets Manager with real values and force an ECS redeploy.
- Marqo semantic search: not deployed.
- File uploads: written to ephemeral container disk — lost on any
  backend redeploy. See `TODO.md`.
